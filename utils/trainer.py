...
class VARTrainer(object):
    def __init__(
            ...
            vae_local: VQVAE, var_local: VAR, var_ddp: DDP,
            var_opt: AmpOptimizer, label_smooth: float,
    ):
        super(VARTrainer, self).__init__()

        self.var_ddp, self.vae_local, self.quantize_local = var_ddp, vae_local, vae_local.quantize
        self.quantize_local: VectorQuantizer2
        self.var_local: VAR = var_local  # after torch.compile
        ...
        self.first_l = self.var_local.first_l
        self.L = self.var_local.L - self.var_local.first_l + 1
    ...
    def test_ep(self, ld_test: DataLoader, ep: int, out_dir: Path):
        ...
        for i, data in enumerate(ld_test):
            ...
            lq_res = self.var_local.autoregressive_infer_cfg(lq)
            hq_res = self.var_local.autoregressive_infer_cfg(hq)
    ...
    def eval_ep(self, ld_val: DataLoader):
        ...
        stt = time.time()
        training = self.var_local.training
        self.var_local.eval()
        ...
        for i, data in enumerate(ld_val):
            ...
            gt_idx_Bl: List[torch.LongTensor] = self.vae_local.img_to_idxBl(hq)
            x_BLCv_wo_first_l = self.quantize_local.idxBl_to_var_input(gt_idx_Bl)
            gt_BL = torch.cat(gt_idx_Bl, dim=1)

            self.var_local.forward
            logits_BLV = self.var_local(lq, x_BLCv_wo_first_l)
    ...
    def train_step(
        self, it: int, g_it: int, stepping: bool, metric_lg: MetricLogger, tensor_board: TensorboardLogger,
        lq: torch.FloatTensor, hq: torch.FloatTensor,
    ) -> Tuple[Optional[Union[torch.Tensor, float]], Optional[float]]:
        # forward
        B, V = lq.shape[0], self.vae_local.vocab_size
        self.var_ddp.require_backward_grad_sync = stepping
        ...
        with self.var_opt.amp_ctx:
            logits_BLV = self.var_ddp(lq, x_BLCv_wo_first_l)
        ...
        if g_it == 0 or (g_it + 1) % 500 == 0:
            ...
            if dist_utils.is_master():
                if g_it == 0:
                    tensor_board.update(head='AR_iter_loss', z_voc_usage=cluster_usage, step=-10000)
                    tensor_board.update(head='AR_iter_loss', z_voc_usage=cluster_usage, step=-1000)
                ...
                tensor_board.update(head='AR_iter_loss', **kw, step=g_it)
    ...
    def state_dict(self):
        ...
        for k in ('var_local', 'vae_local', 'var_opt'):
            m = getattr(self, k)
            if m is not None:
    ...
    def load_state_dict(self, state, strict=True, skip_vae=False):
        if len(state) == 1:
            ...
            ks = ('var_local', 'vae_local', 'var_opt')
        for k in ks:
